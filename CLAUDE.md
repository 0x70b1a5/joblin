...
- **Buttons must answer within 3s**: a handler that does Discord I/O before responding needs `press.ack()` (defer) first; quick refusals use `press.whisper(...)` (ephemeral). Prefer riding the free interaction response (`press.edit_pressed`, `interaction.response.edit_message`) so an action costs one API call, and never respond twice.
